...
public:
    ...
    {
    public:
        bool operator() (const TMS_Entity* entity1, const TMS_Entity* entity2) const
        {
    ...
    /***************** METHODS *****************/
    bool addEntity(TMS_Entity* entity); // Add a single entity to the dispatcher.
    bool addEntities(const std::vector<TMS_Entity*>& entities); // Add multiple entities to the dispatcher.

    std::optional<TMS_Action> dispatchEvent(const SDL_Event& event, const TMS_EventData& eventData,
                                            const glm::fvec2 cameraPos);

    void removeEntity(TMS_Entity* entity); // Remove a specific entity from the dispatcher.
...
private:
    ...
    std::set<TMS_Entity*, EntityComparator> _entities[static_cast<int>(tms::EventType::TOT)];
};
